...
export async function POST(request) {
  try {
    ...
    var fileMime = body.fileMime || "text/plain";

    if (!resumeText && !fileBase64) {
      return NextResponse.json({ error: "Please provide resume text or a file." }, { status: 400 });
    }
    ...
    var promptText = "Parse this resume and extract structured data. Return ONLY valid JSON with no markdown formatting or preamble. Use this exact structure:\n\n";
    ...
    promptText += "For headline, create a brief one-line professional summary.\n";
    promptText += "For summary, write 2-3 sentences capturing the candidate career trajectory and key strengths.\n";
    promptText += "For education, combine all degrees into one string.\n";
    promptText += "For certifications, combine all.\n";
    promptText += "If a field has no data, leave it as an empty string.\n";
    ...
    var cleaned = text.replace(new RegExp(bt3+"json","g"), "").replace(new RegExp(bt3,"g"), "").trim();
