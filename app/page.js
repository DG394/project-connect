...
function EngProfile({engId, onBack, onNav, people}) {
  ...
  return (<div><BackBtn onClick={onBack} label="Engagements" />
    <div className="flex items-start justify-between mb-6">
      <div>
        ...
        <p className="text-sm text-[#8891A5]">
          <span onClick={()=>{const co=eng.company_name;onNav("companies",null,co)}} className="text-[#4F8BF5] hover:underline cursor-pointer">{eng.company_name}</span>
      ...
      <Card className="mb-5">
        <ProfileSection title={"Candidates ("+cands.length+")"} action={
          <div className="flex gap-2 items-center">
            ...
            <AddBtn label="Add Candidate" onClick={()=>setShowAddCand(true)} />
          </div>
        }>
          {candData.length===0 ? <EmptyState text="No candidates added yet." /> : <SortTable data={candData} cols={CAND_COLS} visKeys={candCols} sortKey={candSort.key} sortDir={candSort.dir} onSort={k=>{if(candSort.key===k)setCandSort(s=>({...s,dir:s.dir==="asc"?"desc":"asc"}));else setCandSort({key:k,dir:"asc"})}} onRowClick={r=>r.person_id&&onNav("people",r.person_id)} renderCell={(row,key)=>{
            if(key==="full_name") return (<span className="font-semibold text-[#4F8BF5]">{row.full_name}</span>);
            if(key==="status") return (<select value={row.status} onClick={e=>e.stopPropagation()} onChange={e=>updCandStatus(row.id,e.target.value)} className="bg-transparent border border-[#252B3A] rounded px-1.5 py-0.5 text-xs text-[#E2E6EF] cursor-pointer outline-none">{CAND_STATUSES.map(s=><option key={s} value={s}>{s}</option>)}</select>);
    ...
    <SlidePanel open={showAddCand} onClose={()=>setShowAddCand(false)} title="Add Candidate to Search">
      <p className="text-sm text-[#8891A5] mb-4">Search for an existing person to add as a candidate.</p>
      <FormField label="Search People">
        <AutoSuggest value={candSearch} onChange={setCandSearch} onSelect={s=>addCand(s.id)} suggestions={pplSugg} placeholder="Start typing a name..." />
      </FormField>
    </SlidePanel>
  </div>);
}
...
function LauncherInputs({data, setData, onComplete, coSugg}) {
  ...
  const autoInfer=async()=>{if(!d.getSmart&&!d.kickoffNotes&&!d.draftJD)return;setInferring(true);setInferErr("");
    try{const text="Based on these search documents, extract the following fields. Return ONLY valid JSON:\n{\"roleName\":\"\",\"companyName\":\"\",\"peFirm\":\"\",\"location\":\"\",\"reportingTo\":\"\",\"teamSize\":\"\",\"compRange\":\"\",\"confidential\":\"yes or no\"}\n\nGET SMART:\n"+(d.getSmart||"N/A")+"\n\nDRAFT JD:\n"+(d.draftJD||"N/A")+"\n\nKICK-OFF NOTES:\n"+(d.kickoffNotes||"N/A");
      const res=await callAI([{role:"user",content:text}]);const cleaned=res.replace(new RegExp(String.fromCharCode(96,96,96)+"json","g"),"").replace(new RegExp(String.fromCharCode(96,96,96),"g"),"").trim();const match=cleaned.match(/\{[\s\S]*\}/);
...
export default function App() {
  ...
  return <>
    <style dangerouslySetInnerHTML={{__html:"*{margin:0;padding:0;box-sizing:border-box}body{background:#0B0E14;color:#E2E6EF;font-family:Instrument Sans,DM Sans,sans-serif;-webkit-font-smoothing:antialiased}::-webkit-scrollbar{width:5px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:#313848;border-radius:3px}@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}}@keyframes shimmer{0%{background-position:-400px 0}100%{background-position:400px 0}}.animate-pulse{animation:pulse 1.5s infinite}.shimmer-line{background:linear-gradient(90deg,#252B3A 25%,#313848 50%,#252B3A 75%);background-size:800px;animation:shimmer 1.5s infinite linear}"}}/>
